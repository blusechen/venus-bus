...
public class SimpleAuthenticateProvider<T extends VenusFrontendConnection> extends AuthenticateProvider<T, byte[]> {
    ...
    public void beforeAuthing(T conn) {
        ...
        handshake.version = PacketConstant.VENUS_VERSION;
        conn.setSeed(handshake.challenge);
        conn.write(handshake.toByteBuffer());
    }

    @Override
    public void handle(T conn, byte[] message) {
